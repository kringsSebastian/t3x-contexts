...
abstract class AbstractContext
{
    ...
    final public function getSettings($table, $uid)
    {
		...
		$rows = $queryBuilder
			...
			->where(
				$queryBuilder->expr()->eq('context_uid', $queryBuilder->createNamedParameter($this->uid, \PDO::PARAM_INT)),
				$queryBuilder->expr()->eq('foreign_table', $queryBuilder->createNamedParameter($table)),
				$queryBuilder->expr()->in('foreign_uid', $queryBuilder->createNamedParameter($$uids, Connection::PARAM_INT_ARRAY))
			)
			->execute()
			->fetch();
