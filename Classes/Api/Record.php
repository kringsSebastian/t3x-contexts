 ...
 */
class Record implements LoggerAwareInterface
{
     ...
     * @return bool
     */
    public function isEnabled($table, $row)
    {
     ...
     * @return bool
     */
    public function isSettingEnabled($table, $setting, $row)
    {
        if (is_array($row)) {
            ...
            if (!isset($row['uid'])) {
                $this->logger->error(
                    'Missing uid field in row: '. 'tx_contexts',
     ...
     *                      boolean otherwise
     */
    protected function isSettingEnabledFlat($table, $setting, array $row)
    {
        $flatColumns
        ...
        foreach ($flatColumns as $i => $flatColumn) {
            if (!array_key_exists($flatColumn, $row)) {
                $this->logger->error(
                    'Missing flat field "' . $flatColumn . '" in '.'tx_contexts',
                    array('table' => $table, 'row' => $row)
                );
                $rowValid = false;
