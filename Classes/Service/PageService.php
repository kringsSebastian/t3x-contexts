 ...
 */
class PageService
    implements PageRepositoryGetPageHookInterface, AbstractMenuFilterPagesHookInterface, SingletonInterface, LoggerAwareInterface
{
    ...
    protected function getFilterSql($table)
    {
        ...
        foreach (Configuration::getEnableSettings($table) as $setting) {
            ...
            if (!$flatColumns) {
                $this->logger->error(
                    'Missing flat columns for setting "' . $setting . '" in table '.
                    'tx_contexts',
                    array('table' => $table)
                );
                continue;
...
//            	Requires further update according to DBAL, is rather a workaround to get SQL string
            	$enableChecks[] = '('.$flatColumns[1].' LIKE \'%,'.(int)$context->getUid().',%\' OR '.$flatColumns[1].' LIKE \''.(int)$context->getUid().',%\' OR '.$flatColumns[1].' LIKE \'%,'.(int)$context->getUid().'\' OR '.$flatColumns[1].'=\''.(int)$context->getUid().'\')';
//                /* @var $context AbstractContext */
//                $enableChecks[] = $GLOBALS['TYPO3_DB']->listQuery(
//                    $flatColumns[1], $context->getUid(), $table
//                );

                $disableChecks[] = 'NOT ('.$flatColumns[0].' LIKE \'%,'.(int)$context->getUid().',%\' OR '.$flatColumns[0].' LIKE \''.(int)$context->getUid().',%\' OR '.$flatColumns[0].' LIKE \'%,'.(int)$context->getUid().'\' OR '.$flatColumns[0].'=\''.(int)$context->getUid().'\')';
//					'NOT ' . $GLOBALS['TYPO3_DB']->listQuery(
