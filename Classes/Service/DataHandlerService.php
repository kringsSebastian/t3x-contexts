...
class DataHandlerService
{
    ...
    protected function saveRecordSettings($table, $uid, $contextsAndSettings)
    {
        ...
        $queryBuilder = GeneralUtility::makeInstance(ConnectionPool::class)->getQueryBuilderForTable('tx_contexts_settings');
        ...
        foreach ($contextsAndSettings as $contextId => $settings) {
            foreach ($settings as $field => $setting) {
				...
				$row = $queryBuilder
					...
					->where(
						$queryBuilder->expr()->eq('context_uid', $queryBuilder->createNamedParameter($contextId, \PDO::PARAM_INT),
							$queryBuilder->expr()->eq('foreign_table', $queryBuilder->createNamedParameter($table)),
							$queryBuilder->expr()->eq('name', $queryBuilder->createNamedParameter($field)),
...
//                );
                if ($setting === '0' || $setting === '1') {
                    if ($row) {
                    	$queryBuilder->update('tx_contexts_settings')
							->where(
    ...
    protected function saveFlatSettings($table, $uid, $contextsAndSettings)
    {
        ...
        if (count($values)) {
            ...
            $queryBuilder
				...
				->set($values, \Doctrine\DBAL\Connection::PARAM_STR_ARRAY)
				->execute();
//            Deprecated
...
//            );
    ...
    protected function saveDefaultSettings($contextId, $settings)
    {
    	/** @var QueryBuilder $queryBuilder */
		$queryBuilder = GeneralUtility::makeInstance(ConnectionPool::class)->getQueryBuilderForTable('tx_contexts_settings');
		$existingSettings = $queryBuilder
			->select('*')
			->from('tx_contexts_settings')
			->where(
				$queryBuilder->expr()->eq('context_uid', $queryBuilder->createNamedParameter($contextId, \PDO::PARAM_INT)),
				$queryBuilder->expr()->eq('foreign_uid', $queryBuilder->createNamedParameter(0, \PDO::PARAM_INT))
			)
			->execute()
			->fetchAll();
//		Deprecated
            ...
            foreach ($fields as $field => $enabled) {
                if (array_key_exists($field, $fieldSettings)) {
                	$queryBuilder
						...
						->where(
							$queryBuilder->expr()->eq('uid', $queryBuilder->createNamedParameter($fieldSettings[$field], \PDO::PARAM_INT))
						)
